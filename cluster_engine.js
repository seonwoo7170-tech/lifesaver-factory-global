...
async function genThumbnail(meta, model) {
    try {
        ...
        ctx.font = 'bold 28px "NanumGothic", "Apple SD Gothic Neo", "Malgun Gothic", sans-serif';
        ...
        ctx.fillStyle = '#ffffff'; ctx.font = 'bold 82px "NanumGothic", "Apple SD Gothic Neo", "Malgun Gothic", sans-serif';
        ...
        ctx.fillStyle = '#cbd5e1'; ctx.font = '36px "NanumGothic", "Apple SD Gothic Neo", "Malgun Gothic", sans-serif';
...
async function genPinterest(meta, model) {
    try {
        ...
        const tag = (meta.tag || 'EXCLUSIVE').toUpperCase();
        ctx.font = 'bold 28px "NanumGothic", "Apple SD Gothic Neo", sans-serif';
        const padding = 20;
        const tagWidth = ctx.measureText(tag).width;
        ...
        ctx.fillStyle = '#ffffff'; ctx.font = 'bold 78px "NanumGothic", "Apple SD Gothic Neo", sans-serif';
        const mainTitle = meta.mainTitle || 'No Title';
        const words = mainTitle.split(' ');
        let line1 = '', line2 = '';
        if(words.length > 3) {
...
async function writeAndPost(model, target, lang, blogger, bId, pTime, extraLinks = [], idx, total, searchQuery = '', globalArchives = []) {
    ...
    const langTag = `\\n\\n[TARGET_LANGUAGE]: ${lang === 'ko' ? 'Korean' : 'English'}\\n[STRICT_MODE]: Write EVERYTHING including Title (<h1>), Body, SEO Meta, and FAQ in ${lang === 'ko' ? 'KOREAN' : 'ENGLISH'}. \\n★ NEVER include language prefixes like 'ko:' or 'en:' in the title. \\n★ If the target language is English, do not use any Korean characters even if the input keyword is in Korean.`;
    report(`🔥 [포스팅 ${idx}/${total}]: '${target}' 집필 및 발행 시작...`);
    const m1 = await callAI(model, MASTER_GUIDELINE + '\\n[MISSION: PART 1] ' + target + '의 최상단 썸네일(IMG_0), TOC, 그리고 🎯전체 6~8개 H2 섹션 중 절반 이상(최소 4~5개)의 방대한 핵심 본문을 매우 상세하게 작성하라. \\n★ 너는 반드시 첫 머리에 파격적인 스타일의 <h1> 제목을 [TARGET_LANGUAGE]에 맞추어 생성해야 한다. (절대 입력 키워드를 그대로 복붙하지 마라)\\n' + searchData + clusterContext1 + '\\n★ 제약: 반드시 HTML 태그가 완벽하게 닫힌 상태에서 PART 1을 종료할 것. ★절대 주의: FAQ나 결론 섹션은 지금 작성하지 마세요.' + langTag);
    report(`   - 미션 1 완료 (${m1.length}자)`);
    let cleanM1 = m1.replace(/\`\`\`(html|json|javascript|js)?/gi, '').replace(/\n네, 이어서.*?하겠습니다\./gi, '').trim();
    const m2 = await callAI(model, MASTER_GUIDELINE + '\\n[이전 파트 1 결과 요약]: \\n' + cleanM1.slice(-1200) + '\\n\\n[MISSION: PART 2] (매우 중요) 위 파트 1의 내용에 끊기지 않고 바로 이어지도록 나머지 후반부 본문(남은 H2 섹션 2~3개)과 FAQ, 결론(마무리 박스)을 상세히 작성하라. (분량 비중 35~45%)\\n' + clusterContext2 + archiveContext + '\\n★ [중복 가드]: 앞서 작성된 파트 1의 내용을 절대 중복해서 다시 쓰지 마십시오. 특히 서두, IMG_0~3 메타데이터, TOC(목차)는 이미 파트 1에 존재하므로 절대로 다시 생성해서는 안 됩니다. 파트 1의 마지막 문장에서 곧바로 이어지는 다음 <h2> 태그 본문부터 순수 HTML 코드만 출력하세요.\\n★ 마크다운(```html) 금지.' + langTag);
    ...
    let cleanM2 = m2.replace(/\`\`\`(html|json|javascript|js)?/gi, '')
                    ...
                    .replace(/<div(?:\s+[^>]*)?>\s*<p[^>]*>\s*(?:Table of Contents|핵심 요약 목차|목차)\s*<\/p>[\s\S]*?<\/div>/gi, '')
                    .trim();
    ...
    if (m0 && !finalHtml.includes('[[IMG_0]]')) {
        finalHtml = '[[IMG_0]]<br>' + finalHtml;
    }
    // [IMG_0 썸네일 및 핀터레스트 이미지 생성 및 치환]
    if (m0 && (finalHtml.includes('[[IMG_0]]') || finalHtml.includes('[IMG_0]'))) {
        ...
        finalHtml = finalHtml.replace(/\\\\??\\[\\\\??\\[IMG_0\\\\??\\]\\\\??\\]/gi, insertHtml);
    }
...
async function run() {
    ...
    // 1단계: 이번 회차 작업 모드 및 키워드 선정
    const mode = config.post_mode || 'cluster';
    let baseKeyword = '';
    ...
    if (mode === 'single') {
        ...
        if (!Array.isArray(clusterList) || clusterList.length < 5) {
            report('⚠️ 클러스터 생성 실패하여 단일 포스팅으로 대체합니다.', 'warning');
            await writeAndPost(model, baseKeyword, config.blog_lang, blogger, config.blog_id, new Date(), [], 1, 1, '', globalArchives);
        } else {
